const editorVisual = (() => {
  ...
  function canonicalizeAssetUrl(value) {
    ...
    if (/^https?:\/\//i.test(trimmed) || trimmed.startsWith("//")) {
      try {
        ...
        if (
          LOCAL_ASSET_HOSTS.has(parsed.hostname) &&
  ...
  function canonicalizeGalleryList(items) {
    ...
    return items.map((item) =>
      canonicalizeAssetUrl(typeof item === "string" ? item : "")
    );
  }
  ...
  function canonicalizeSiteMedia() {
    ...
    state.siteState.hero_image = canonicalizeAssetUrl(
      state.siteState.hero_image || ""
    );
    state.siteState.about_image = canonicalizeAssetUrl(
      state.siteState.about_image || ""
    );
    state.siteState.products = canonicalizeProductsList(
      state.siteState.products || []
    );
  ...
  function renderPaletteSuggestions(modelId = state.siteState.model_type) {
    ...
    if (typeof getPalettesForModel !== "function" || !modelId) {
      container.innerHTML = "";
      if (helper)
        helper.textContent = "Selecciona un modelo para ver sus paletas.";
      return;
    }
    ...
    if (!palettes.length) {
      ...
      if (helper)
        helper.textContent =
          "Este modelo no tiene paletas curadas, usa los selectores de color manualmente.";
      return;
    }
                ...
                <button type="button" class="palette-card ${
                  isActive ? "palette-card-active" : ""
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        ${
                          isActive
                            ? '<span class="text-[10px] font-semibold text-blue-500 bg-blue-50 px-2 py-0.5 rounded-full">Aplicada</span>'
                            : ""
                        }
                    </div>
  ...
  function updatePreviewModeUI() {
    ...
    document
      .querySelectorAll("#previewModeToggle [data-preview-mode]")
      .forEach((button) => {
  ...
  async function loadModels() {
    ...
    select.innerHTML = state.models
      .map(
        (model) =>
          `<option value="${model.id}">${getModelIcon(model.id)} ${
            model.name
          }</option>`
      )
      .join("");
  ...
  function updateMeta(site) {
    ...
    if (statusEl)
      statusEl.textContent = site.is_published ? "Publicado" : "Borrador";
    if (modelEl) modelEl.textContent = getModelName(site.model_type);
    if (updatedEl)
      updatedEl.textContent = site.updated_at
  ...
  function renderProducts() {
    ...
    container.innerHTML = state.siteState.products
      .map(
        (product, index) => `
            <article class="border border-gray-100 rounded-2xl p-4 space-y-3 bg-gray-50">
                <div class="flex items-start justify-between">
                    <h4 class="text-sm font-semibold text-gray-700">Producto ${
                      index + 1
                    }</h4>
                ...
                <input type="text" class="input-control" value="${escapeAttr(
                  product.name
                )}" placeholder="Nombre" oninput="editorVisual.updateProduct(${index}, 'name', this.value)">
                <textarea class="input-control" rows="2" placeholder="Descripción" oninput="editorVisual.updateProduct(${index}, 'description', this.value)">${escapeText(
          product.description
        )}</textarea>
                <div class="dual-inputs">
                    <input type="text" class="input-control" value="${escapeAttr(
                      product.price
                    )}" placeholder="Precio" oninput="editorVisual.updateProduct(${index}, 'price', this.value)">
                    <div class="space-y-2">
                        <div class="flex gap-2">
                            <input type="url" class="input-control flex-1" value="${escapeAttr(
                              product.image
                            )}" placeholder="Imagen (URL)" oninput="editorVisual.updateProduct(${index}, 'image', this.value)">
  ...
  function renderGallery() {
    ...
    if (
      !state.siteState.gallery_images ||
      state.siteState.gallery_images.length === 0
    ) {
      container.innerHTML = "";
      empty.classList.remove("hidden");
      return;
    ...
    empty.classList.add("hidden");
    container.innerHTML = state.siteState.gallery_images
      .map(
        (url, index) => `
            <div class="flex items-center gap-2">
                <input type="url" class="input-control flex-1" value="${escapeAttr(
                  url
                )}" placeholder="https://" oninput="editorVisual.updateGalleryImage(${index}, this.value)">
...
(function(){
  ...
  function validateImageFile(file) {
    ...
    if (!ACCEPTED_IMAGE_TYPES.includes(file.type)) {
      showNotification(
        "Formato no soportado. Usa PNG, JPG, WEBP, GIF o SVG.",
        "error"
      );
      return false;
    }
  ...
  async function publishSite() {
    const confirmed = await confirmAction(
      "¿Deseas publicar este sitio en GitHub Pages?",
      {
    ...
    if (!saved) {
      showNotification(
        "No se pudo guardar el sitio antes de publicar",
        "error"
      );
      return;
    }
    ...
    try {
      const response = await fetchAPI(`/api/sites/${state.siteId}/publish`, {
        method: "POST",
      });
      const data = await response.json();
      if (response.ok) {
